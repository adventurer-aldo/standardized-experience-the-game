...
func _ready():
	...
	if subject_key == 0:
		...
		for subject in keys:
			...
			var subj_level = clampi(1 + int((float(subject.experience) / float(subject.maximum_experience)) * 9), 1, 10)
			clone.text = "Lv. " + str(subject.get_level()) + ": " + subject.title
...
func ost():
	match quiz.level:
		...
		"engage_test1_4"].pick_random()
		1: return ["fates_exam_0", "fates_test1_0", "awakening_practice",
		...
		4: return ["awakening_exam", "houses_test2_0"].pick_random() #"engage_exam0"
...
func _on_time_rush_time_started() -> void:
	BGM.autoplay_rush("engage_rush2_0")
